 ...
 */
public class SlightlySoupCopy {

    private Logger log = LoggerFactory.getLogger(SlightlySoupCopy.class);
    private ScriptEngine engine;

    public SlightlySoupCopy() {
        engine = new ScriptEngineManager().getEngineByName("nashorn");
        log = LoggerFactory.getLogger(SlightlySoupCopy.class);
    }

    public String evalElement(String html, HttpServletRequest request)  {
            ...
            public void head(Node node, int depth) {
                ...
                if (node instanceof Element) {
                    ...
                    attributes.forEach(attr -> {
                        attr.setValue(resolveExpression(attr.getValue()));
                    });
                    ...
                    dataset.forEach((k, v) -> {
                        if (k.startsWith("for-")) {
                            ...
                            try {
                                ...
                                element.html("");
                                //For each item in items, set the implicitItem variable and then evaluate the resolveExpression
                                items.forEach(item -> {
                                    Element temp = dummy.clone();
                                    session.put(implicitItem, item);
                                    temp.html(resolveExpression(temp.text())); // Evaluate the resolveExpression
                    ...
                    //Replace this textNodes text with the result of the resolveExpression evaluation from the Nashorn engine
                    textNode.text(resolveExpression(textNode.text()));
                } else {
    ...
    public String resolveExpression(String html) {
        Pattern el$resolveExpression = Pattern.compile("\\$\\{(\\w+)\\.?(\\w+)\\}");
        //String html ="<h1 title=\"${person.name}\">${person.name} ${person}</h1>";
        log.info("$Expression: " + html);
        Matcher action = el$resolveExpression.matcher(html);
